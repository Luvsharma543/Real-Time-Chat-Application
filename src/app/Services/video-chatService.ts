...
export class VideoChatService {
  ...
  sendOffer(receiverId:string,offer:RTCSessionDescriptionInit){
    this.hubConnection.invoke("SendOffer",receiverId,JSON.stringify(offer)).catch(err=>{
  ...
  sendEndCall(receiverId:string){
    this.hubConnection.invoke("EndCall",receiverId).catch(err=>{
      console.log("Error while sending end call: "+err);
    });
  }
}
